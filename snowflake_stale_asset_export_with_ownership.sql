 ...
 *   - OWNERSHIP_STATUS: OWNED | PARTIALLY_OWNED | ORPHANED
 *   - SNOWFLAKE_DATA_STATUS: ACTIVE | RESTRICTED lifecycle evidence
 *
 * PRD ownership rule:
 *   An asset is ORPHANED only when both DPMT and BDST assignments are absent.
 *   Technical ownership and stewardship are retained as useful routing metadata,
 *   but do not independently satisfy the DPMT/BDST ownership requirement.
 *******************************************************************************/
...
joined AS (
    SELECT
        ...
        CASE
            WHEN NULLIF(g.sf_dpm_team, '') IS NULL
             AND NULLIF(g.sf_bds_team, '') IS NULL
                THEN 'ORPHANED'
            WHEN NULLIF(g.sf_dpm_team, '') IS NULL
              OR NULLIF(g.sf_bds_team, '') IS NULL
                THEN 'PARTIALLY_OWNED'
            ELSE 'OWNED'
